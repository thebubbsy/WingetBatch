...
function Install-WingetAll {
    ...
    .PARAMETER UseFob
        Use FindObject (fob) module for advanced filtering with AND/OR/NOT logic.
        Requires FindObject module to be installed.
    ...
    .EXAMPLE
        Install-WingetAll "python", "not", "2.7" -UseFob
        Uses FindObject to find Python packages but exclude version 2.7.

    .EXAMPLE
        Install-WingetAll "office", "or", "word" -UseFob
        Uses FindObject to find packages containing "office" OR "word".
    ...
    begin {
        $allCollectedPackages = @()

        # If no SearchTerm provided, we'll list ALL winget packages for piping
        $listAll = $false
        if (-not $SearchTerm -or $SearchTerm.Count -eq 0) {
            $listAll = $true
            $PassThru = $true  # Output mode for piping to fob
        }
        # Check if we're receiving piped package objects
        elseif ($SearchTerm.Count -gt 0 -and $SearchTerm[0].PSObject.TypeNames -contains 'WingetBatch.Package') {
            # We're receiving piped packages - don't process, just collect
            # This will be handled in process block
        }
        else {
            ...
            elseif ($SearchTerm.Count -eq 1 -and $SearchTerm[0] -is [string]) {
                # Already an array with one string element - keep as is
            }
            else {
                # Ensure it's a proper array of strings
                $SearchTerm = @($SearchTerm | ForEach-Object { $_.ToString() })
            }
        }

        # Check if UseFob requires FindObject module
        if ($UseFob) {
            if (-not (Get-Module -ListAvailable -Name FindObject)) {
                Write-Error "FindObject module is required when using -UseFob. Install it with: Install-Module -Name FindObject"
                return
            }
            Import-Module FindObject -ErrorAction Stop
            Write-Host "Using FindObject (fob) for advanced filtering" -ForegroundColor Magenta
        }

        # Check if PwshSpectreConsole is available (only if not in PassThru mode)
        if (-not $PassThru) {
        ...
        if (-not $listAll) {
            ...
            Write-Host ($SearchTerm -join ' ') -ForegroundColor Yellow
        }
        else {
            Write-Host "Listing ALL winget packages for pipeline filtering..." -ForegroundColor Cyan
        }
    }

    process {
        # If receiving piped package objects, collect them for end block
        if ($SearchTerm) {
            # Check if this is a package object from pipeline
            foreach ($item in $SearchTerm) {
                if ($item.PSObject.TypeNames -contains 'WingetBatch.Package') {
                    $allCollectedPackages += $item
                }
            }
            # If we collected packages, skip the search processing
            if ($allCollectedPackages.Count -gt 0) {
                return  # Continue collecting in process, handle in end
            }
        }
        ...
        if ($listAll) {
            try {
                # List ALL packages with empty search
                $searchResults = winget search "" --accept-source-agreements 2>&1
                if ($LASTEXITCODE -ne 0) {
                    Write-Error "Error listing all packages."
                    return
                }
            }
            catch {
        ...
        # Determine search strategy based on UseFob
        elseif ($UseFob) {
            # When using fob, search broadly and let fob filter
            # Use first non-operator term for broad search
            $broadSearchTerm = $SearchTerm | Where-Object { $_ -notmatch '^(and|or|not)$' } | Select-Object -First 1

            if (-not $broadSearchTerm) {
                Write-Error "No valid search term found. Cannot search without at least one keyword."
                return
            }

            try {
                $searchResults = winget search $broadSearchTerm --accept-source-agreements 2>&1
                if ($LASTEXITCODE -ne 0) {
                ...
                Write-Error "Failed to search for packages: $_"
                return
            }
        }
        else {
            # Original behavior: Parse individual search words for wildcard searching
            # Flatten array and split by whitespace
            $searchWords = @()
            foreach ($term in $SearchTerm) {
                $searchWords += $term -split '\s+' | Where-Object { $_ -ne '' }
            }
        ...
        $packageIds = @()
        $packageMap = @{}  # Store Name->Id mapping
        $headerFound = $false
        $nameColEnd = -1
        $idColStart = -1
        $idColEnd = -1
                ...
                if ($packageId -and $packageId -match '^[A-Za-z0-9\.\-_]+$' -and $packageId -notmatch '^\d+\.\d+') {
                    ...
                    if ($searchWords.Count -gt 1) {
                        ...
                        if ($matchesAll) {
                            ...
                            $packageMap[$packageId] = $packageName
                        }
                    }
                    else {
                        $packageIds += $packageId
        ...
        if ($packageIds.Count -eq 0) {
            Write-Warning "No packages found matching '$(if ($UseFob) { $SearchTerm -join ' ' } else { $SearchTerm })'"
            return
        }

        $packageIds = @($packageIds | Select-Object -Unique)

        # Apply FindObject filtering if requested
        if ($UseFob) {
            Write-Host "Applying FindObject filter..." -ForegroundColor Magenta

            # Create package objects with Name property for fob
            $packageObjects = $packageIds | ForEach-Object {
                [PSCustomObject]@{ Name = $_ }
            }

            # Apply fob filter
            try {
                $filteredObjects = $packageObjects | Find-ObjectByName -SearchTerms $SearchTerm
                $packageIds = @($filteredObjects | ForEach-Object { $_.Name })

                if ($packageIds.Count -eq 0) {
                    Write-Warning "No packages matched FindObject filter"
                    return
                }
            }
            catch {
                Write-Error "FindObject filtering failed: $_"
                return
            }
        }
        ...
        # PassThru mode: output package objects for pipeline
        if ($PassThru) {
            Write-Host "Outputting packages to pipeline..." -ForegroundColor Magenta
            foreach ($id in $packageIds) {
                $name = if ($packageMap.ContainsKey($id)) { $packageMap[$id] } else { $id }
                [PSCustomObject]@{
                    PSTypeName = 'WingetBatch.Package'
                    Name = $name
                    Id = $id
                }
            }
        ...
        # Handle collected packages from pipeline (e.g., after fob filtering)
        if ($allCollectedPackages.Count -gt 0) {
            Write-Host "`nReceived " -ForegroundColor Cyan -NoNewline
            Write-Host "$($allCollectedPackages.Count)" -ForegroundColor White -NoNewline
            Write-Host " package(s) from pipeline" -ForegroundColor Cyan

            $packageIds = @($allCollectedPackages | ForEach-Object { $_.Id })

            if ($WhatIf) {
                Write-Host "`n[WhatIf] Would install:" -ForegroundColor Yellow
                $packageIds | ForEach-Object {
                    Write-Host "  • " -ForegroundColor Cyan -NoNewline
                    Write-Host $_ -ForegroundColor White
                }
            ...
            # Ensure PwshSpectreConsole is loaded for interactive selection
            if (-not $Silent) {
            ...
            # Interactive selection or install
            if (-not $Silent -and (Get-Module -Name PwshSpectreConsole)) {
                Write-Host ""
                try {
                    $selectedPackages = Read-SpectreMultiSelection -Title "[cyan]Select packages to install[/]" `
                    ...
                    Write-Warning "Interactive selection failed, installing all piped packages"
                }
            }
...
function Format-WingetSelection {
    <#
    .SYNOPSIS
        Display winget packages in interactive selection menu and install.

    .DESCRIPTION
        Collects package objects from pipeline, displays in Spectre Console selection menu,
        and installs selected packages. Designed to work with Install-WingetAll | fob pipeline.

    .PARAMETER InputObject
        Package objects from pipeline.

    .PARAMETER Silent
        Skip interactive selection and install all.

    .PARAMETER WhatIf
        Show what would be installed without installing.

    .EXAMPLE
        Install-WingetAll | fob "python 3.13" | Format-WingetSelection
        Automatically shows selection menu after filtering.
    #>
    ...
    process {
        # Collect all piped package objects
        if ($InputObject.PSObject.TypeNames -contains 'WingetBatch.Package') {
            $collectedPackages += $InputObject
        }
        else {
            # Also handle regular objects with Name/Id properties
            $collectedPackages += $InputObject
        }
    }

    end {
        if ($collectedPackages.Count -eq 0) {
            Write-Warning "No packages to display"
            return
        }

        Write-Host "`n📦 Found " -ForegroundColor Cyan -NoNewline
        Write-Host "$($collectedPackages.Count)" -ForegroundColor White -NoNewline
        Write-Host " package(s)" -ForegroundColor Cyan

        $packageIds = @($collectedPackages | ForEach-Object { $_.Id })
        ...
        if (-not $Silent) {
            if (Get-Module -Name PwshSpectreConsole) {
                ...
                try {
                    ...
                    Write-Warning "Interactive selection failed: $_"
                    Write-Host "`nPackages to install:" -ForegroundColor Yellow
                    $packageIds | ForEach-Object { Write-Host "  • $_" -ForegroundColor White }
...
Set-Alias -Name fws -Value Format-WingetSelection

function Invoke-WingetInstall {
    <#
    .SYNOPSIS
        Install winget packages from pipeline (e.g., after fob filtering).

    .DESCRIPTION
        Receives package objects from pipeline and shows interactive selection menu,
        then installs selected packages.

    .PARAMETER InputObject
        Package objects from pipeline (e.g., from Install-WingetAll | fob).

    .PARAMETER Silent
        Skip interactive selection and install all piped packages.

    .PARAMETER WhatIf
        Show what would be installed without installing.

    .EXAMPLE
        Install-WingetAll | fob "python 3.13" | Invoke-WingetInstall
        Filter for Python 3.13, show selection menu, install selected.

    .EXAMPLE
        Install-WingetAll | fob "python", "not", "2.7" | iwi -Silent
        Filter and install without confirmation using alias.
    #>
    ...
    process {
        ...
        if ($collectedPackages.Count -eq 0) {
            Write-Warning "No packages received from pipeline"
            return
        }
        ...
        # Load Spectre Console for interactive selection
        if (-not $Silent) {
            if (-not (Get-Module -Name PwshSpectreConsole)) {
                Import-Module PwshSpectreConsole -ErrorAction SilentlyContinue
            }
        ...
        if (-not $Silent) {
            if (Get-Module -Name PwshSpectreConsole) {
                ...
                # Fallback confirmation when Spectre Console not available
                Write-Host "`nPackages to install:" -ForegroundColor Yellow
                $packageIds | ForEach-Object { Write-Host "  • $_" -ForegroundColor White }
                ...
                if ($selectedChoices.Count -gt 0) {
                    ...
                    foreach ($job in $relevantJobs) {
                        if ($job.State -eq 'Completed') {
                            ...
                            Write-Host "   [DEBUG] Job $($job.Id) returned type: $($jobResults.GetType().Name)" -ForegroundColor Yellow
                            Write-Host "   [DEBUG] Job $($job.Id) has $($jobResults.Count) packages" -ForegroundColor Yellow
                            if ($jobResults -is [hashtable] -and $jobResults.Count -gt 0) {
                                $firstKey = $jobResults.Keys | Select-Object -First 1
                                $firstValue = $jobResults[$firstKey]
                                Write-Host "   [DEBUG] Sample package: $firstKey" -ForegroundColor Yellow
                                Write-Host "   [DEBUG] Sample fields: $($firstValue.Keys -join ', ')" -ForegroundColor Yellow
                            }
                    ...
                    # DEBUG: Show what's in master hashtable
                    Write-Host "   [DEBUG] Master hashtable has $($allPackageDetails.Count) packages" -ForegroundColor Yellow
                    if ($allPackageDetails.Count -gt 0) {
                        $sampleKey = $allPackageDetails.Keys | Select-Object -First 1
                        $sampleValue = $allPackageDetails[$sampleKey]
                        Write-Host "   [DEBUG] Sample master package: $sampleKey" -ForegroundColor Yellow
                        Write-Host "   [DEBUG] Sample master fields: $($sampleValue.Keys -join ', ')" -ForegroundColor Yellow
                    }
                    ...
                    # DEBUG: Show what's in extracted package details
                    Write-Host "   [DEBUG] Extracted $($packageDetails.Count) package details" -ForegroundColor Yellow
                    if ($packageDetails.Count -gt 0) {
                        $extractedKey = $packageDetails.Keys | Select-Object -First 1
                        $extractedValue = $packageDetails[$extractedKey]
                        Write-Host "   [DEBUG] Sample extracted package: $extractedKey" -ForegroundColor Yellow
                        Write-Host "   [DEBUG] Sample extracted fields: $($extractedValue.Keys -join ', ')" -ForegroundColor Yellow
                        Write-Host "   [DEBUG] Sample Version value: $($extractedValue.Version)" -ForegroundColor Yellow
                        Write-Host "   [DEBUG] Sample Publisher value: $($extractedValue.Publisher)" -ForegroundColor Yellow
                    }
...
function ConvertFrom-WingetUpgradeRow {
    <#
    .SYNOPSIS
        Parse a single winget upgrade table row.

    .DESCRIPTION
        Internal helper that converts a line from `winget upgrade` table output into
        a structured object containing name, id, installed/available versions, and source.
    #>
    ...
    if ($trimmedLine -match '^Name\s+Id\s+' -or
        $trimmedLine -match '^-{3,}' -or
        $trimmedLine -match '(?i)upgrades? available' -or
        $trimmedLine -match '(?i)no package\(s\) found') {
        return $null
    }
    ...
    if (-not $id -and $DefaultId) {
        $id = $DefaultId
    }

    if (-not $id -or $id -notmatch '^[A-Za-z0-9][A-Za-z0-9\.\-_]*$') {
        return $null
    }

    $installed = if ($parts.Count -ge 3) { $parts[2].Trim() } else { '' }
    $available = if ($parts.Count -ge 4) { $parts[3].Trim() } else { '' }
    $source = if ($parts.Count -ge 5) { $parts[4].Trim() } else { '' }

    if ($parts.Count -eq 3 -or ($available -eq '' -and $source -eq '')) {
        $extraTokens = $parts[2].Split(' ', [System.StringSplitOptions]::RemoveEmptyEntries)
        if ($extraTokens.Count -ge 1) { $installed = $extraTokens[0] }
        if ($extraTokens.Count -ge 2) { $available = $extraTokens[1] }
        if ($extraTokens.Count -ge 3) { $source = $extraTokens[2] }
    }
    elseif ($parts.Count -eq 4 -and $source -eq '') {
        $extraTokens = $parts[3].Split(' ', [System.StringSplitOptions]::RemoveEmptyEntries)
        if ($extraTokens.Count -ge 1 -and $available -eq '') { $available = $extraTokens[0] }
        if ($extraTokens.Count -ge 2) { $source = $extraTokens[1] }
    }
...
function ConvertFrom-WingetUpgradeOutput {
    <#
    .SYNOPSIS
        Parse winget upgrade command output into structured objects.
    #>
    ...
    foreach ($line in $Output -split "`r?`n") {
        ...
        $record = ConvertFrom-WingetUpgradeRow -Line $line
        if ($null -ne $record -and $seenIds.Add($record.Id)) {
            $results.Add($record)
        }
    }
...
function ConvertFrom-WingetUpgradeCacheRecord {
    <#
    .SYNOPSIS
        Normalize cached upgrade data into the current record structure.
    #>
    ...
    $hasDisplayLine = $Record.PSObject.Properties.Name -contains 'DisplayLine'
    if ($hasDisplayLine) {
        return ConvertFrom-WingetUpgradeRow -Line $Record.DisplayLine -DefaultId $Record.Id
    }

    $name = if ($Record.PSObject.Properties.Name -contains 'Name') { [string]$Record.Name } else { '' }
    $id = if ($Record.PSObject.Properties.Name -contains 'Id') { [string]$Record.Id } else { '' }
    $installed = if ($Record.PSObject.Properties.Name -contains 'InstalledVersion') { [string]$Record.InstalledVersion } else { '' }
    $available = if ($Record.PSObject.Properties.Name -contains 'AvailableVersion') { [string]$Record.AvailableVersion } else { '' }
    $source = if ($Record.PSObject.Properties.Name -contains 'Source') { [string]$Record.Source } else { '' }
    $raw = if ($Record.PSObject.Properties.Name -contains 'Raw') { [string]$Record.Raw } else {
        ($name, $id, $installed, $available, $source | Where-Object { $_ -and $_.Trim() -ne '' }) -join '  '
    }
...
function Invoke-WingetUpgradeCommand {
    <#
    .SYNOPSIS
        Execute the winget upgrade command and capture output and exit code.
    #>
    ...
    $textOutput = if ($output -is [System.Array]) { $output -join [Environment]::NewLine } else { [string]$output }
...
function Start-WingetUpdateCheck {
    ...
    $job = Start-Job -ScriptBlock {
        ...
        try {
            ...
            $upgradeRaw = winget upgrade --disable-interactivity 2>&1
            $upgradeText = if ($upgradeRaw -is [System.Array]) { $upgradeRaw -join "`n" } else { [string]$upgradeRaw }
            ...
            $parseRow = {
                ...
                if ($trim -match '^Name\s+Id\s+' -or
                    $trim -match '^-{3,}' -or
                    $trim -match '(?i)upgrades? available' -or
                    $trim -match '(?i)no package\(s\) found') {
                    return $null
                }

                $parts = $trim -split '\s{2,}' | Where-Object { $_ -ne '' }
                if ($parts.Count -lt 2) {
                    return $null
                }
            ...
            foreach ($line in $upgradeText -split "`r?`n") {
                $record = & $parseRow $line
                if ($record -and -not $seen.ContainsKey($record.Id)) {
                    $updatesAvailable += $record
                    $seen[$record.Id] = $true
                }
            }
            ...
            $cache | ConvertTo-Json -Depth 5 | Out-File -FilePath $cacheFile -Encoding UTF8 -Force
...
function Get-WingetUpdates {
    ...
    .PARAMETER IncludeUnknown
        Include packages where the installed version cannot be determined.
    ...
    .EXAMPLE
        ...
        Includes packages where winget reports the installed version as unknown.
    #>
    ...
    if (-not $Force -and (Test-Path $cacheFile)) {
        try {
            ...
            if ($cache) {
                ...
                $cacheIncludeUnknown = [bool]$cache.IncludeUnknown

                if ($cacheAge -lt 30 -and $cacheIncludeUnknown -eq $IncludeUnknown.IsPresent) {
                    $converted = @()
                    foreach ($item in $cache.Updates) {
                        $record = ConvertFrom-WingetUpgradeCacheRecord -Record $item
                        if ($record) {
                            $converted += $record
                        }
                    }
            ...
            Write-Verbose "Failed to read update cache: $_"
        }
    }

    if (-not $useCache) {
        $commandResult = Invoke-WingetUpgradeCommand -IncludeUnknown:$IncludeUnknown.IsPresent
        $updatesAvailable = ConvertFrom-WingetUpgradeOutput -Output $commandResult.Output

        if ($commandResult.ExitCode -ne 0) {
            Write-Warning "winget upgrade exited with code $($commandResult.ExitCode). Showing parsed results."
        }
        ...
        try {
            $cachePayload | ConvertTo-Json -Depth 5 | Out-File -FilePath $cacheFile -Encoding UTF8 -Force
        }
        catch {
            Write-Verbose "Failed to write update cache: $_"
        }
    }
    ...
    if ($IncludeUnknown) {
        Write-Host "Including packages where the installed version is unknown." -ForegroundColor DarkGray
    }

    Write-Host ""

    $displayLookup = [ordered]@{}
    $baseTextCounts = @{}

    foreach ($update in $updatesAvailable) {
        $baseText = if ($update.Raw) {
            $update.Raw
        }
        else {
            "{0}  {1}  {2}  {3}  {4}" -f $update.Name, $update.Id, $update.InstalledVersion, $update.AvailableVersion, $update.Source
        }

        if ($baseTextCounts.ContainsKey($baseText)) {
            $baseTextCounts[$baseText] = [int]$baseTextCounts[$baseText] + 1
            $displayText = "{0} [{1}]" -f $baseText, $baseTextCounts[$baseText]
        }
        else {
            $baseTextCounts[$baseText] = 1
            $displayText = $baseText
        }

        $displayLookup[$displayText] = $update
    }

    $displayLines = [string[]]$displayLookup.Keys
    ...
    if (Get-Module -Name PwshSpectreConsole) {
        try {
            ...
            $selectedUpdates = $selectedLines | ForEach-Object { $displayLookup[$_] }

            Write-Host ""
            Write-Host "Updating " -ForegroundColor Cyan -NoNewline
            Write-Host "$($selectedUpdates.Count)" -ForegroundColor White -NoNewline
            Write-Host " package(s)..." -ForegroundColor Cyan
            Write-Host ""
            ...
            foreach ($update in $selectedUpdates) {
                ...
                Write-Host "$($update.Name) [$($update.Id)]" -ForegroundColor White

                winget upgrade --id $update.Id --accept-package-agreements --accept-source-agreements

                if ($LASTEXITCODE -eq 0) {
                    Write-Host "✓ Successfully updated " -ForegroundColor Green -NoNewline
                    Write-Host "$($update.Id)" -ForegroundColor White
                    $successCount++
                }
                else {
            ...
            foreach ($update in $updatesAvailable) {
                $installedVersion = if ($update.InstalledVersion -and $update.InstalledVersion.Trim()) { $update.InstalledVersion } else { 'Unknown' }
                $availableVersion = if ($update.AvailableVersion -and $update.AvailableVersion.Trim()) { $update.AvailableVersion } else { 'Unknown' }
                Write-Host ("  • {0} [{1}] {2} -> {3}" -f $update.Name, $update.Id, $installedVersion, $availableVersion) -ForegroundColor White
            }
            Write-Host ""
            Write-Host "Use 'winget upgrade <PackageId>' to update manually." -ForegroundColor Yellow
            return
        }
    }
    else {
        ...
        Write-Host "winget upgrade <PackageId>" -ForegroundColor Yellow
...
# Create aliases
Set-Alias -Name iwi -Value Invoke-WingetInstall
...
Export-ModuleMember -Function Install-WingetAll, Invoke-WingetInstall, Format-WingetSelection, Get-WingetNewPackages, `
