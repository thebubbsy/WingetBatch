...
## 2025-10-27 - [Regex -replace overhead in simple string replacements]
**Learning:** In performance-critical PowerShell functions (like `ConvertTo-SpectreEscaped` used during formatting loops), the `-replace` operator incurs unnecessary regular expression compilation and evaluation overhead when only literal string substitution is needed.
**Action:** Use native `.Replace()` string methods instead of `-replace` for literal substitutions. Combine this with early exit checks like `.IndexOf()` to bypass allocations and logic entirely when the target substring isn't present.
