...
## 2026-04-07 - [String Replace Overhead without IndexOf Check]
**Learning:** In PowerShell, using `.Replace()` for literal substitutions incurs memory allocation and processing overhead even if the target character isn't present in the string. For frequently called functions like `ConvertTo-SpectreEscaped` (used heavily during loop iterations for text markup escaping), testing strings that *don't* contain brackets via `.Replace()` takes nearly identical time as those that do. Wrapping the replacements in an early-exit check (`if ($Text.IndexOf('[') -eq -1 -and $Text.IndexOf(']') -eq -1) { return $Text }`) avoids this overhead, dramatically speeding up the "happy path" (no brackets) while maintaining safety.
**Action:** When performing simple literal character replacements in performance-critical paths where the target character is relatively rare, always guard the `.Replace()` call with a `.IndexOf()` or `.Contains()` check to short-circuit the execution and avoid unnecessary operations.
