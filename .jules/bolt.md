...
## 2026-02-14 - [Regex Overhead in Simple String Replacements]
**Learning:** Using the `-replace` operator with regular expressions (e.g., `-replace '\[', '[['`) for simple character or substring replacements in frequently called functions incurs significant regular expression engine compilation and evaluation overhead. Furthermore, executing this replacement when the target characters are not even present wastes CPU cycles and creates unnecessary string allocations.
**Action:** Use native .NET string operations like `.Replace('[', '[[')` instead of `-replace`. Additionally, implement an early-exit "fast path" using `.IndexOf()` to quickly bypass the replacement logic entirely when the target characters are not found in the string.
