...
function Get-WingetNewPackages {
    ...
    try {
        ...
        if ($GitHubToken) {
            ...
            Write-Host "Using stored GitHub token for authentication (5,000 req/hour)" -ForegroundColor DarkGray
        }
        else {
            Write-Host "No GitHub token found (60 req/hour limit)" -ForegroundColor DarkGray
            Write-Host "Tip: Run " -NoNewline -ForegroundColor DarkGray
            Write-Host "Set-WingetBatchGitHubToken" -NoNewline -ForegroundColor Yellow
        ...
        foreach ($commit in $allCommits) {
            ...
            if ($packageName -and -not $processedPackages.ContainsKey($packageName)) {
                try {
                    # Build GitHub manifest URL: manifests/<first-letter>/<publisher>/<appname>/<version>/
                    $parts = $packageName -split '\.'
                    if ($parts.Count -ge 2) {
                        $publisher = $parts[0]
                        $firstLetter = $publisher.Substring(0, 1).ToLower()
                        $manifestPath = "manifests/$firstLetter/$publisher/$($parts[1..($parts.Count-1)] -join '.')/$version"
                        $githubUrl = "https://github.com/microsoft/winget-pkgs/tree/master/$manifestPath"
                    }
                    else {
                        $githubUrl = "https://github.com/microsoft/winget-pkgs/search?q=$packageName"
                    }
...
function Remove-WingetRecent {
    ...
    .DESCRIPTION
        Shows packages installed in the last X hours and allows interactive selection
        of which packages to uninstall.

    .PARAMETER Hours
        Number of hours to look back for recently installed packages. Default is 24 hours.

    .EXAMPLE
        Remove-WingetRecent
        Shows packages installed in the last 24 hours and allows you to select which to uninstall.

    .EXAMPLE
        Remove-WingetRecent -Hours 48
        Shows packages installed in the last 48 hours.
    #>
    ...
    Write-Host "$Hours hour$(if ($Hours -ne 1) { 's' })..." -ForegroundColor Yellow
    ...
    try {
        ...
        foreach ($line in $listLines) {
            ...
            if ($headerFound -and $line.Trim() -ne '' -and $idColStart -gt 0 -and $line.Length -gt $idColStart) {
                ...
                # Only process valid package IDs
                if ($packageId -and $packageId -match '^[A-Za-z0-9\.\-_]+$') {
                    # Try to get installation info using winget show
                    try {
                        $showOutput = winget show --id $packageId --disable-interactivity 2>&1 | Out-String

                        # Look for "Publisher:" line to confirm package exists
                        if ($showOutput -match 'Publisher:') {
                            $installedPackages += @{
                                Id = $packageId
                                DisplayLine = $line.Trim()
                            }
        ...
        if ($installedPackages.Count -eq 0) {
            Write-Warning "No installed packages found. Make sure winget is working correctly."
            return
        }

        Write-Host "Found " -ForegroundColor Green -NoNewline
        Write-Host "$($installedPackages.Count)" -ForegroundColor White -NoNewline
        Write-Host " installed package(s)" -ForegroundColor Green
        Write-Host ""
        Write-Host "Note: " -ForegroundColor Yellow -NoNewline
        Write-Host "Winget doesn't track installation dates. All installed packages are shown." -ForegroundColor DarkGray
        Write-Host ""
...
# Internal functions: Get-WingetBatchGitHubToken, Start-WingetUpdateCheck
