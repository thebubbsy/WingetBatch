...
## Registry-Based Package Removal (Remove-WingetRecent)

### Windows Registry Sources
Reads installation dates from three registry paths:
...
### Date Filtering
- Parameter: `-Days <int>` (e.g., `-Days 7` for last week)
- Reads `InstallDate` property (format: YYYYMMDD)
- Converts to `[datetime]` and compares against current date
- Presents packages sorted by newest first

### Uninstall Execution
Uses registry `UninstallString` or `QuietUninstallString`:
```powershell
if ($quietUninstall) {
    Start-Process cmd.exe -ArgumentList "/c $quietUninstall" -Wait
...
- **Job pooling depends on package count** - fewer than 10 packages creates fewer jobs
- **Job package mapping critical** - `$jobPackageMap` must be maintained to categorize relevant jobs
- **Background jobs must complete or be stopped** - orphaned jobs can accumulate and impact performance
- **Registry InstallDate format varies** - not all packages have this field, some use different date formats
- **Full descriptions can be very long** - removed truncation means output may scroll extensively
