    ...
    try {
        ...
        foreach ($commit in $allCommits) {
            ...
            if ($packageName -and -not $processedPackages.ContainsKey($packageName)) {
                try {
                    # Try to get the package homepage from winget
                    $homepageUrl = $null
                    try {
                        $showOutput = winget show --id $packageName --disable-interactivity 2>&1 | Out-String

                        # Look for Publisher Url or Homepage
                        if ($showOutput -match '(?:Publisher Url|Homepage):\s*(.+)') {
                            $homepageUrl = $matches[1].Trim()
                        }
                        # Fallback to Package Url
                        elseif ($showOutput -match 'Package Url:\s*(.+)') {
                            $homepageUrl = $matches[1].Trim()
                        }
                    }
                    catch {
                        # If winget show fails, we'll use fallback
                    }

                    # Fallback to search if no URL found
                    if (-not $homepageUrl) {
                        $homepageUrl = "https://www.google.com/search?q=$([uri]::EscapeDataString($packageName))"
                    }
