...
function Get-WingetNewPackages {
    ...
    try {
        ...
        foreach ($pkg in $newPackages) {
            $pkgId = $pkg.Name
            $jobs[$pkgId] = Start-Job -ScriptBlock {
                param($packageId)
                ...
                foreach ($line in $output -split "`n") {
                    if ($line -match '^\s*Publisher:\s*(.+)$') { $info.Publisher = $matches[1].Trim() }
                    elseif ($line -match '^\s*Publisher Url:\s*(.+)$') { $info.Homepage = $matches[1].Trim() }
        ...
        Write-Host "   Started $($jobs.Count) background jobs..." -ForegroundColor DarkGray
        ...
        if (Get-Module -Name PwshSpectreConsole) {
            ...
            try {
                ...
                if ($selectedChoices.Count -gt 0) {
                    ...
                    Write-Host ""
                    $runningJobs = @($jobs.Values | Where-Object { $_.State -eq 'Running' })

                    if ($runningJobs.Count -gt 0) {
                        Write-Host "⏳ Waiting for $($runningJobs.Count) background jobs to complete..." -ForegroundColor DarkGray
                        $timeout = 30
                        $runningJobs | Wait-Job -Timeout $timeout | Out-Null
                    }
                    else {
                        Write-Host "✓ All package details already fetched!" -ForegroundColor Green
                    }
                    ...
                    foreach ($pkg in $newPackages) {
                        ...
                        if ($job.State -eq 'Completed') {
                            $allPackageDetails[$pkgId] = Receive-Job -Job $job
                        }
                        else {
                    ...
                    Write-Host "✓ Package details ready" -ForegroundColor Green
                    Write-Host ""
                    ...
                    foreach ($pkgId in $packagesToInstall) {
                        ...
                        if ($pkgInfo.Version) {
                            ...
                            Write-Host $pkgInfo.Version -ForegroundColor White
                        }
                    ...
                    $jobs.Values | Stop-Job | Out-Null
                    $jobs.Values | Remove-Job -Force | Out-Null
                }
            }
            catch {
                ...
                if ($jobs) {
                    $jobs.Values | Stop-Job -ErrorAction SilentlyContinue | Out-Null
                    $jobs.Values | Remove-Job -Force -ErrorAction SilentlyContinue | Out-Null
                }
            }
        }
        else {
...
function Update-GitHubApiRequestCount {
    ...
    if (Test-Path $rateLimitFile) {
        try {
            ...
            $lastReset = [DateTime]::Parse($jsonData.LastReset)
