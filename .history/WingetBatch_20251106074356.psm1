...
# Wrap fob (FindObject) to automatically show selection menu for WingetBatch packages
function fob {
    ...
    begin {
        $allInput = @()
        $isWingetPackages = $false
    }

    process {
        $allInput += $InputObject
        # Check if we're dealing with WingetBatch packages
        if ($InputObject.PSObject.TypeNames -contains 'WingetBatch.Package') {
            $isWingetPackages = $true
        }
    }

    end {
        # Build parameter set for Find-ObjectByName
        $fobParams = @{
            SearchTerms = $SearchTerms
        }
        if ($Property) { $fobParams['Property'] = $Property }
        if ($Regex) { $fobParams['Regex'] = $true }
        if ($CaseSensitive) { $fobParams['CaseSensitive'] = $true }

        # Filter using Find-ObjectByName (pass array to pipeline)
        $filtered = @($allInput | Find-ObjectByName @fobParams)

        # If these are WingetBatch packages, automatically show menu and install
        if ($isWingetPackages -and $filtered.Count -gt 0) {
            $fwsParams = @{}
            if ($Silent) { $fwsParams['Silent'] = $true }
            if ($WhatIfPreference) { $fwsParams['WhatIf'] = $true }

            $filtered | Format-WingetSelection @fwsParams
        }
        else {
            # Just output the filtered results for non-WingetBatch objects
            $filtered
        }
    }
}

# Create aliases - Force override FindObject's fob alias
Set-Alias -Name fob -Value fob -Scope Global -Force
...
Export-ModuleMember -Function Install-WingetAll, Invoke-WingetInstall, Format-WingetSelection, Get-WingetNewPackages, `
    ...
    Get-WingetUpdates, Remove-WingetRecent, fob `
    -Alias iwi, fws
