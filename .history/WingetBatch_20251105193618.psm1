...
function Remove-WingetRecent {
    ...
    try {
        ...
        if ($installedPackages.Count -eq 0) {
            Write-Warning "No packages installed in the last $Hours hour$(if ($Hours -ne 1) { 's' })."
        ...
        Write-Host " package(s) installed in the last $Hours hour$(if ($Hours -ne 1) { 's' })" -ForegroundColor Green
        Write-Host ""
        ...
        if (Get-Module -Name PwshSpectreConsole) {
            try {
                ...
                $displayLines = $installedPackages | ForEach-Object {
                    $dateStr = $_.InstallDate.ToString('yyyy-MM-dd HH:mm')
