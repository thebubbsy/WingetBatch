...
function Install-WingetAll {
    ...
    process {
        ...
        # Handle piped packages
        if ($pipedPackages.Count -gt 0) {
            Write-Host "`nReceived " -ForegroundColor Cyan -NoNewline
            Write-Host "$($pipedPackages.Count)" -ForegroundColor White -NoNewline
            Write-Host " package(s) from pipeline" -ForegroundColor Cyan

            $packageIds = @($pipedPackages | ForEach-Object { $_.Id })
