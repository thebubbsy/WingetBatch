        ...
        if (Get-Module -Name PwshSpectreConsole) {
            ...
            try {
                ...
                if ($selectedChoices.Count -gt 0) {
                    ...
                    # Wait for background jobs to complete (they've been running during user selection)
                    Write-Host ""
                    Write-Host "⏳ Finalizing package details..." -ForegroundColor DarkGray
                    $timeout = 30
                    $jobs.Values | Wait-Job -Timeout $timeout | Out-Null
