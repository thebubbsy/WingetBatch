@{
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = @'
v2.1.0 - Update Experience Improvements
- ENHANCED: Get-WingetUpdates now parses winget output into structured records with installed/available versions, richer console output, and on-disk caching that respects the --include-unknown flag.
- NEW: Get-WingetUpdates -IncludeUnknown parameter exposes winget's --include-unknown switch.
- ENHANCED: Background Start-WingetUpdateCheck job writes the new cache format for consistent notifications.
- ENHANCED: Install-WingetAll removes duplicate package identifiers prior to prompting/installation.
